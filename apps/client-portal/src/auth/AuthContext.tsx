...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function login(input: LoginInput) {
    ...
    if (!response.user.memberships.some((m) => m.role === 'CLIENT_USER')) {
      throw new Error('NOT_A_CLIENT_ACCOUNT');
    }
  ...
  const clientId = user?.memberships.find((m) => m.role === 'CLIENT_USER')?.scopeId ?? null;

  return <AuthContext.Provider value={{ user, status, clientId, login, logout }}>{children}</AuthContext.Provider>;
}
